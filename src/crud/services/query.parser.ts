/**
 * modified https://github.com/tmukammel/sequelize-query-parser
 */
 ...
 */
module.exports = function (Op, models) {
	const operators = {
		gt: Op.gt,
		gte: Op.gte,
		lt: Op.lt,
	...
	const iterativeReplace = (json) => {
		Object.keys(json).forEach(function (key) {
			...
			} else if (key == 'model' && models[json[key]] != null) {
				// json['as'] = json[key].replace(/^./, (char) => char.toLowerCase()); // /^\w/
				json['model'] = models[json[key]];
			} else {
				const op = operators[key];
